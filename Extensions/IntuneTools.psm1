    ...
    try
    {
        ...
        if($addToDefaultAppsPolicies -and $targetGroup -and $targetGroup.Id)
        {
            ...
            if($parentGroup -and $parentGroup.Id)
            {
                ...
                if($isAlreadyMember)
                {
                    Show-InfoMessage "Group '$groupName' is already a member of '$defaultAppsAndPoliciesGroupName'."
                    return
                }
                ...
                $memberAdded = $updatedParentMembers | Where-Object { $_.id -eq $targetGroup.Id } | Select-Object -First 1
                if($memberAdded)
                {
